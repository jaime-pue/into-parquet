...
import com.github.jaime.intoParquet.model.{PairCSVAndTableDescription, ParsedObjectWrapper}
...
class TestController extends SparkTestBuilder {
    ...
    test("Should iterate over all files even if error") {
        val goodFile = new PairCSVAndTableDescription("exampleTable", None)
        val badFile  = new PairCSVAndTableDescription("badFile", None)
