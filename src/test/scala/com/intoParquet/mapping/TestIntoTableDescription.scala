...
class TestIntoTableDescription extends AnyFunSuite {
    ...
    test("Should turn a string into a seq") {
        ...
        assertResult(expected)(FromStringToTableDescription.splitInLines(example))
    }
    ...
    test("Should return only field data") {
        ...
        assertResult(expected)(FromStringToTableDescription.deleteFirstLine(example))
    }
    ...
    test("Should return field data if no comment") {
        ...
        assertResult(example)(FromStringToTableDescription.deleteFirstLine(example))
    }

    test("Should work with regex") {
        val example = "name type comment"
        assert(FromStringToTableDescription.matchCase(example))
    }
    ...
    test("Should regex with multiple spaces") {
        ...
        assert(FromStringToTableDescription.matchCase(twoSpaces))
    }
    ...
    test("Should work if header is longer") {
        ...
        assert(FromStringToTableDescription.matchCase(header))
    }
}
